...
1.  Do glucose levels differ between inpatients and outpatients?
    ...
    ```{r}
      ...
      glu_subset <- filter(chem, test == 'glucose', 
                           pat_type %in% c('inpatient', 'outpatient'))
      
      t_test(glu_subset, response = value, explanatory = pat_type,
             order = c('inpatient', 'outpatient'))
    ```
...
```{r}
  ...
  outpt_iron_subset <- filter(chem, test == 'iron', pat_type == 'outpatient')
  
  t_test(outpt_iron_subset, response = age, explanatory = gender,
         order = c('Male', 'Female'))
```
...
Is the gender split different for different patient types?

```{r}
  # Convert gender to a binary variable
  chem <- chem |>
    mutate(gender_binary = if_else(gender == 'Male', TRUE, FALSE))
  
  chisq_test(chem, gender_binary ~ pat_type)
```
